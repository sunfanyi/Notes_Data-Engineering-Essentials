...
**注意：**
- 首日初始化对实时同步和离线同步的意义时完全不同的！
- 实时同步：
  - 创建实时同步是不会记录已有数据的，只有在新增或者修改的时候才会有新数据进去；
  - 所以需要额外的一张表格记录下以往的数据，上图中如果采用实时同步每个表后缀都要加上`_init`；
  - 后续维度的表格不需要再额外建`_init`表，因为后续都是用 SQL 写的，写 SQL 的时候同样需要初始化，但直接在首日初始化都是时候把以往数据加到要用的表里就行了。
- 离线同步：
  - 离线同步的后续更新都是通过 SQL 的，实际上是直接在已有的表格假数据的过程，所以首日初始化不用在表后缀加`_init`，后续直接往这个表上加数据就行。
